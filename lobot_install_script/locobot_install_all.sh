...
if [ $INSTALL_TYPE == "full" ]; then
	...
	if [ ! -d "$CAMERA_FOLDER/src/realsense-ros" ]; then
		cd $CAMERA_FOLDER/src/
    		git clone https://github.com/peterchenyipu/realsense-ros.git
		cd realsense-ros/
		git checkout pyrobot
	fi
...
if [ ! -d "$LOCOBOT_FOLDER/src/pyrobot" ]; then
	cd $LOCOBOT_FOLDER/src
	git clone https://github.com/peterchenyipu/pyrobot.git
	cd pyrobot
	git checkout main
...
if [ ! -d "$LOCOBOT_FOLDER/src/pyrobot/robots/LoCoBot/thirdparty" ]; then
  	...
  	cd thirdparty
		...
		git clone https://github.com/peterchenyipu/ORB_SLAM2.git
	...
	if [ $ROS_NAME == "kinetic" ]; then
		...
		cd ORB_SLAM2 && git checkout pyrobot && cd ..
